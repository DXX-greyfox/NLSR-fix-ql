 ...
 */
 ...
 public:
  ...
  struct LinkMetrics {
   ...
   struct OutgoingLinkState {
     ...
     static constexpr size_t PACKET_WINDOW_SIZE = 15; // 窗口大小：最近10次
 ...
 private:
  ...
  /**
   * @brief 处理来自nlsrc的get-probe-metrics命令
   * @details 返回基于RTT探测得到的当前RTT和丢包率，用于外部监控/可视化。
   */
  void handleGetProbeMetricsCommand(const ndn::Interest& interest);
