...
ConfFileProcessor::processConfSectionGeneral(const ConfigSection& section)
{
  ...
  try {
    ...
    if (boost::iequals(qLearningStr, "off") || boost::iequals(qLearningStr, "false")) {
      m_confParam.setQLearningRouting(false);
    }
    else if (boost::iequals(qLearningStr, "on") || boost::iequals(qLearningStr, "true")) {
      m_confParam.setQLearningRouting(true);
    }
    else {
      std::cerr << "Invalid value for q-learning-routing: " << qLearningStr << std::endl;
      return false;
    }
  }
  catch (const std::exception& ex) {
