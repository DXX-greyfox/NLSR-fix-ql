...
# 用于存储文件的最后修改时间
g_last_topo_mod_time = 0.0
g_last_metrics_mod_time = 0.0 
...
def initialize_paths():
    ...
    在服务器启动时初始化所有文件路径。
    """
    global topology_json_path, metrics_json_path, g_last_topo_mod_time, g_last_metrics_mod_time
    ...
    metrics_json_path = Path(state_dir) / METRICS_FILE_NAME
    
    # 4. 初始化文件修改时间
    try:
        g_last_topo_mod_time = os.path.getmtime(topology_json_path)
    except FileNotFoundError:
        g_last_topo_mod_time = 0.0
    ...
    try:
        g_last_metrics_mod_time = os.path.getmtime(metrics_json_path)
    except FileNotFoundError:
        g_last_metrics_mod_time = 0.0
...
def get_topology_long_poll():
    ...
    监视 topology.json 和 metrics.json 文件的变化。
    """
    global g_last_topo_mod_time, g_last_metrics_mod_time
    ...
    while time.time() - start_time < POLL_TIMEOUT_SEC:
        try:
            # 检查两个文件的当前修改时间
            current_topo_mod = os.path.getmtime(topology_json_path) if os.path.exists(topology_json_path) else 0.0
            current_metrics_mod = os.path.getmtime(metrics_json_path) if os.path.exists(metrics_json_path) else 0.0
            
            # 如果任一文件时间戳已更新
            if current_topo_mod > g_last_topo_mod_time or current_metrics_mod > g_last_metrics_mod_time:
                g_last_topo_mod_time = current_topo_mod
                g_last_metrics_mod_time = current_metrics_mod
                ...
                if os.path.exists(metrics_json_path):
                    ...
                    topo_data["directed_links"] = metrics_data.get("directed_links", [])
                else:
                    topo_data["directed_links"] = []
                ...
                for node in topo_data.get('nodes', []):
                    ...
                    name_to_id[short_name] = node.get('id')
                
                # 将 directed_links 中的名称转换为数字 ID
                for link in topo_data.get('directed_links', []):
                    src_short = link['src'].split('/')[-1].lower()
                    dst_short = link['dst'].split('/')[-1].lower()
                    link['src_id'] = name_to_id.get(src_short, -1)
                    link['dst_id'] = name_to_id.get(dst_short, -1)
                
                print(f"Info: 检测到文件更新，发送拓扑 (nodes:{len(topo_data.get('nodes',[]))}, links:{len(topo_data.get('links',[]))}, directed:{len(topo_data['directed_links'])})", file=sys.stderr)
